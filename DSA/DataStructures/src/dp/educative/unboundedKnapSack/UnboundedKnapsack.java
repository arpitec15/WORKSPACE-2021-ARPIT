...
//		return knapsackRecursiveMemo(dp, profits, weights, capacity, 0);

		return knapsackDP(profits, weights, capacity);
	}

	private static int knapsackDP(int[] profits, int[] weights, int capacity) {
...
// 		maximum profit will be in the bottom-right corner.
	...
	private static int knapsackRecursiveMemo(Integer[][] dp, int[] profits, int[] weights, int capacity,
			int currentIndex) {
		...
		if (dp[currentIndex][capacity] == null) {
			...
			if (weights[currentIndex] <= capacity)
				profit1 = profits[currentIndex]
						+ knapsackRecursiveMemo(dp, profits, weights, capacity - weights[currentIndex], currentIndex);

			// recursive call after excluding the element at the currentIndex
			int profit2 = knapsackRecursiveMemo(dp, profits, weights, capacity, currentIndex + 1);
